...
import checkCookieExpires from '../../support/check/checkCookieExpires';
...
Given(/^I open the (url|site) "([^"]*)?"$/, openUrl);
...
Given(
    /^the cookie "([^"]*)?" has an expiration (exactly|at least|no more than) "([^"]*)?" days from now$/,
    checkCookieExpires
);

Given(
    /^the cookie "([^"]*)?" has an expiration at the end of the session$/,
    function(name) {
        checkCookieExpires(name);
    }
);
