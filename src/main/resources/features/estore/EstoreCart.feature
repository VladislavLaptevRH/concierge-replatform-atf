...
Feature: Estore Cart Page
  ...
  Scenario: Place order for guest user
    ...
    When I choose country for eStore from footer
    When I open product page with "prod13800635" and "17050044" with "NGRH" for estore
  ...
  Scenario Outline: Gift cards on all brands
  ...
  Scenario: Verify the membership fee - US and CAN fee
    ...
    When I open estore cart
    When I click on join now membership button
    Then I verify order estimate section in xcart
    When I click on remove membership estore button
  ...
  Scenario: Verify postal code update in address page and then in cart and pdp
    ...
    Then I verify that postal code should be equal to 28801
