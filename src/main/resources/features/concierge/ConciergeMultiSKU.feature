...
Feature: Multi SKU
 ...
 Scenario: Verify user is able to see the measuring units in inches for size/length options for NA region
  ...
  When I choose country for concierge from footer
  When I remove all items from cart via UI
  When I go to concierge item 'prod21350057' from search field
  When I click on the first project search result with parameters 'prod21350057''m00007693524'
  Then I Verify that 'PDP title' is present
  Then I chose the '1' line item selections one by one
 ...
 Scenario: Verify user is able to see the measuring units in CM for size/length options for UK region
  ...
  When I remove all items from cart via UI
  When I go to item "SOFA SECTIONAL" from search field
  Then I chose '5' product on the page
  Then I Verify that 'PDP title' is present
  Then I chose the '1' line item selections one by one
